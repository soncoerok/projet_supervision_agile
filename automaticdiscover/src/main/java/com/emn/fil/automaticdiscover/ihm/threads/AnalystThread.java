...
public class AnalystThread implements Runnable{
	...
	public void run(){
			...
			if(IP.isValidIP(myFrame.getTxtboxFrom().getText()) && IP.isValidIP(myFrame.getTxtboxTo().getText())){
				...
				while (range.hasNext()) {
					...
					if(os_current != OsType.UNKNOWN){
						...
						myFrame.getTaConsole().append(" => AJOUTE\r\n");
					}else
						myFrame.getTaConsole().append(" => NOK\r\n");
					
					myFrame.getProgressBar().setValue(nb_current);
					myFrame.getLblPercent().setText((nb_current*100)/nb_ips+"%");
					nb_current++;
					range.next();
				}
